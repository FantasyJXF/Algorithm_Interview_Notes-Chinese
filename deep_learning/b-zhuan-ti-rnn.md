...
* [RNN 的基本结构](b-zhuan-ti-rnn.md#rnn-的基本结构)
* [RNN 常见的几种设计模式（3）](b-zhuan-ti-rnn.md#rnn-常见的几种设计模式3)
* [RNN 的反向传播（BPTT） TODO](b-zhuan-ti-rnn.md#rnn-的反向传播bptt-todo)
* [RNN 相关问题](b-zhuan-ti-rnn.md#rnn-相关问题)
  * [RNN 相比前馈网络/CNN 有什么特点？](b-zhuan-ti-rnn.md#rnn-相比前馈网络cnn-有什么特点)
  * [RNN 为什么会出现梯度消失/梯度爆炸？](b-zhuan-ti-rnn.md#rnn-为什么会出现梯度消失梯度爆炸)
    * [RNN 中能否使用 `ReLU` 作为激活函数？](b-zhuan-ti-rnn.md#rnn-中能否使用-relu-作为激活函数)
    * [如果使用 `ReLU` 作为 RNN 的激活函数，应该注意什么？](b-zhuan-ti-rnn.md#如果使用-relu-作为-rnn-的激活函数应该注意什么)
    * [梯度爆炸的解决方法](b-zhuan-ti-rnn.md#梯度爆炸的解决方法)
    * [梯度消失的解决方法（针对 RNN）](b-zhuan-ti-rnn.md#梯度消失的解决方法针对-rnn)
* [LSTM 相关问题](b-zhuan-ti-rnn.md#lstm-相关问题)
  * [LSTM 的内部结构](b-zhuan-ti-rnn.md#lstm-的内部结构)
    * [完整的 LSTM 前向传播公式](b-zhuan-ti-rnn.md#完整的-lstm-前向传播公式)
  * [LSTM 是如何实现长短期记忆的？（遗忘门和输入门的作用）](b-zhuan-ti-rnn.md#lstm-是如何实现长短期记忆的遗忘门和输入门的作用)
  * [LSTM 里各部分使用了不同的激活函数，为什么？可以使用其他激活函数吗？](b-zhuan-ti-rnn.md#lstm-里各部分使用了不同的激活函数为什么可以使用其他激活函数吗)
  * [窥孔机制](b-zhuan-ti-rnn.md#窥孔机制)
  * [GRU 与 LSTM 的关系](b-zhuan-ti-rnn.md#gru-与-lstm-的关系)
    * [完整的 GRU 前向传播公式](b-zhuan-ti-rnn.md#完整的-gru-前向传播公式)

## RNN 的基本结构

* RNN 本质上是一个**递推函数** 
* 考虑当前输入 `x(t)` 
* 以上计算公式可展开为如下**计算图**（无输出单元） ![](../.gitbook/assets/tim-jie-tu-20180829145622%20%281%29.png)
* RNN 的**前向传播**公式 
...
* **展开后的 RNN**（**无输出**） ![](../.gitbook/assets/tim-jie-tu-20180808110903%20%281%29.png)
...
* 答案是肯定的。但是会存在一些问题。
  ...
  > 其实 ReLU 最早就是为了解决 RNN 中的**梯度消失**问题而设计的。 &lt;!-- - 使用 ReLU 主要是为了解决**反向传播**时的**梯度消失**问题；
...
* LSTM （上）与传统 RNN（下） 的内部结构对比 ![](../.gitbook/assets/lstm3-chain%20%281%29.png) ![](../.gitbook/assets/lstm3-simplernn.png)
